%%%%%%%%%%%%%%%%%%%%%%%%
% Main code for obtain concentration information from processed frames.
% By Rui Luo 20171112
clear;
clc;
close all;

% Number of frames to be calculate concentration.
%------------------------------------------------------------------------------------
numFrame=[90];  
phi = 0.31;
intial_x_crop = 410;
...
color = char('b','r','k','g','m','c','y','b','r','k','g','m','c');
pictureLoc =[ 'C:\Users\Rui\Desktop\phi',num2str(phi*100),'\pics2'];
dataLoc =[ 'C:\Users\Rui\Desktop\phi',num2str(phi*100),'\data points'];
outputLoc=['C:\Users\Rui\Desktop\phi',num2str(phi*100),'%_',date,'.xlsx'];
titleMPlot = ['concentration profile at \phi_0 =', num2str(phi)];
[~,numCalMax]=size(numFrame);
C=180;
cropvec = [intial_x_crop intial_y_crop x_max y_max];
image_ave = IntCrct(cropvec);

fname = sprintf('finger_%i.png',5);
fnamet=sprintf('finger_%i.mat',5);
...
%fitting process
    fname = sprintf('finger_%i.mat',60);
    fig = load(fullfile(dataLoc,fname)); 
    
    fname2 = sprintf('finger_%i.png',60);
...
for mCal = 1:numCalMax
    [a,concentration,location,error] = CntrCal(C,numFrame(mCal),image_ave,dataLoc,pictureLoc,color,ref_in1,ref_in2,fitting_int,phi);
    hold on
    clear x1 y1 I I_corrected
    output1={'location(m)','concentration(%)','error'};
    output2=[location',concentration',error'];
    output3={'data of frame',num2str(numFrame(mCal))};
    sheetName=['Sheet',num2str(mCal)];
    xlswrite(outputLoc,output1,sheetName,'A1');
    xlswrite(outputLoc,output2,sheetName,'A2');
    xlswrite(outputLoc,output3,sheetName,'D1')
    plot(location,concentration,color(mCal),'MarkerSize',15,'LineWidth',3);
    hold on
end
